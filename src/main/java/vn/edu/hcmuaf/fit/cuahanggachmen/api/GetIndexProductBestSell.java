...
@WebServlet(name = "GetIndexProductBestSell", value = "/loadmoresell")
public class GetIndexProductBestSell extends HttpServlet {
...
//        NumberFormat currencyFormat = NumberFormat.getCurrencyInstance(userLocale);
        ...
        list = pd.findTopSellingProducts(1,3, iamount);

        PrintWriter out = response.getWriter();
        if(list.size() > 0) {
            // Bắt đầu khối popular mới
            out.println("<div class=\"popular\" id=\"popular1\">");
