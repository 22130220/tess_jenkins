...
@WebServlet(name = "GetIndexProductBestView", value = "/loadmoreview")
public class GetIndexProductBestView extends HttpServlet {
...
//        NumberFormat currencyFormat = NumberFormat.getCurrencyInstance(userLocale);
        ...
        list = pd.findMostViewedProducts(iamount);

        PrintWriter out = response.getWriter();
        if(list.size() > 0) {
            // Bắt đầu khối popular mới
            out.println("<div class=\"popular\" id=\"popular3\">");
